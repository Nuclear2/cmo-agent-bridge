...
如果没有可用的 `codex` CLI，下载 v0.3.1 Release 中的本地安装脚本，再从磁盘执行：
...
仅注册 MCP 不会安装 Skill。还需从 `v0.3.1` 标签复制完整目录
...
```text
先调用 cmo_bridge_diagnose；需要时在当前任务调用 cmo_bridge_prepare。再调用 cmo_time_get_state：
想定正在运行就直接调用 cmo_bridge_status；想定已经暂停则调用
cmo_simulation_pulse(handshake=true)。确认当前 CMO build、runtime tag 和想定 lineage。
```

暂停分支会由 Agent 以 1x 短暂释放时间，握手完成后自动复停并恢复原倍率，不需要玩家手动操作。
