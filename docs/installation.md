...
> `v0.1.3` 是 **Preview / GitHub Pre-release**，不是稳定正式版。请先备份想定，并优先在测试副本
...
Invoke-WebRequest `
  -UseBasicParsing `
  -Uri "https://github.com/Nuclear2/cmo-agent-bridge/releases/download/v0.1.3/install-codex-desktop.ps1" `
  -OutFile $installer
...
```powershell
git clone --depth 1 --branch v0.1.3 https://github.com/Nuclear2/cmo-agent-bridge.git
cd cmo-agent-bridge
```
...
```text
uvx --python 3.12 --from https://github.com/Nuclear2/cmo-agent-bridge/releases/download/v0.1.3/cmo_agent_bridge-0.1.3-py3-none-any.whl cmo-bridge serve
```
