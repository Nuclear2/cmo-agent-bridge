...
> `v0.7.0` 是 **Preview / GitHub Pre-release**，不是稳定正式版。请先备份想定，并优先在测试副本
...
- `cmo_time_set(state, rate_code?)`：幂等地暂停、恢复或选择倍率；`rate_code` 0–3 对应 1x、2x、
  5x、15x，4–5 是一秒与五秒粗粒度火焰模式，实际速度取决于机器性能；
...
```powershell
$skillZip = Join-Path $env:TEMP "operate-cmo-skill-0.7.0.zip"
$skillRoot = Join-Path $env:TEMP "operate-cmo-skill-0.7.0"
Invoke-WebRequest `
  -UseBasicParsing `
  -Uri "https://github.com/Nuclear2/cmo-agent-bridge/releases/download/v0.7.0/operate-cmo-skill-0.7.0.zip" `
  -OutFile $skillZip
...
```powershell
git clone --depth 1 --branch v0.7.0 https://github.com/Nuclear2/cmo-agent-bridge.git
cd cmo-agent-bridge
```
...
```text
uvx --python 3.12 --from https://github.com/Nuclear2/cmo-agent-bridge/releases/download/v0.7.0/cmo_agent_bridge-0.7.0-py3-none-any.whl cmo-bridge serve
```
...
无法获得这一修复；因此从 0.6.1 或更早版本升级时，必须先退出旧客户端/会话，再启动新版本。

如果隔离请求所属的原想定仍可打开，应使用产生该请求的版本运行普通 `resolve-quarantine`，并依据
原想定中的实际结果选择 `applied` 或 `not_applied`。只有在请求来自旧 bridge release、原想定已经
永久废弃、当前加载的是全新想定，而且你明确确认旧操作应视为未执行时，才使用离线维护入口：

```powershell
$preview = cmo-bridge abandon-obsolete-quarantine `
  --game-root "D:\Program Files (x86)\Steam\steamapps\common\Command - Modern Operations" |
  ConvertFrom-Json

# 核对 request_id、operation、required_release_id 和 disposition=not_applied 后：
cmo-bridge abandon-obsolete-quarantine `
  --game-root "D:\Program Files (x86)\Steam\steamapps\common\Command - Modern Operations" `
  --confirmation-token $preview.confirmation_token
```

该命令不与 CMO 握手，也不会重放请求；它只把精确匹配的旧 release journal、ledger 与可能残留的
同一请求 ACTIVE queue 行一致地收敛为 `not_applied`。确认令牌 60 秒有效；当前 release 的 journal、
任何 queued 请求或其它 active 请求都会被拒绝。不要用手工删除 pending 文件或直接修改 SQLite
来代替它。
