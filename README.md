...
CMO Agent Bridge 将推演和想定制作中常用的 Lua API 整理成 68 个结构化 MCP 工具。Agent
可以读取战场态势、创建任务、分配兵力、规划航路和加油，也可以协助制作单位、天气与事件。
所有通信都留在本机，CMO 侧只需运行一个很短的轮询事件。

> **当前版本是 v0.1.0 预览版。** 已在 Windows、CMO Build 1868 上验证；第一次接入建议使用
> 想定副本。
...
`uvx` 会为 bridge 准备独立的 Python 3.12 环境和运行依赖。
...
然后在 Codex CLI 中打开 `/plugins`，或在 Codex Desktop 的 **Plugins** 页面中，从
**CMO Tools** 安装 `cmo-agent-bridge`。

#### Claude Code

```powershell
claude plugin marketplace add Nuclear2/cmo-agent-bridge
claude plugin install cmo-agent-bridge@cmo-tools --scope user
```

执行 `/reload-plugins` 或新建会话即可载入。

OpenCode、Cursor、Qoder 和通用 MCP 客户端的配置见[各框架安装](docs/frameworks/README.md)。这些
框架使用同一个本地 `stdio` MCP Server。

### 3. 部署 CMO 侧运行时

使用与插件相同的 Release 版本执行 `prepare`：
...
如果 CMO 安装在其他位置，把 `--game-root` 换成实际路径即可。经常使用 CLI 的用户可以按
[安装文档](docs/installation.md)将 `cmo-bridge` 安装为常驻命令。
...
保持 CMO 和目标想定运行，在 PowerShell 中执行：

```powershell
uvx --python 3.12 --from $wheel cmo-bridge invoke bridge.status --args '{}'
```

返回 `"ok": true`，说明 CMO 侧已经接通。接着重启 Agent 或新建会话，输入：

> 调用 `cmo_bridge_status`，告诉我当前 CMO build、runtime tag 和想定 lineage。

如果 CLI 请求一直等待，检查轮询 event 是否启用、是否允许重复，以及想定时间是否正在推进。如果
Agent 中还没有出现 `cmo_*` 工具，重启 Agent 并新建会话。
...
- **MCP Server** 提供 68 个 `cmo_*` 工具，负责读取和修改 CMO 状态；
...
- 当前版本：[`v0.1.0 Preview`](https://github.com/Nuclear2/cmo-agent-bridge/releases/tag/v0.1.0)
